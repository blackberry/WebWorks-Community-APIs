...
module.exports = {
	...
	test: function (success, fail) {
		success(template.getInstance().test());
	},
	...
	testAsyncJSON: function (success, fail) {
		template.getInstance().testAsyncJSON();
		success();
	},
	testAsyncJSONio: function (success, fail, args) {
		args = JSON.parse(decodeURIComponent(args["input"]));
		template.getInstance().testAsyncJSONio(args);
		success();
	},
	templateProperty: function (success, fail, args) {
		...
		if (args && args["value"]) {
			...
			template.getInstance().templateProperty(value);
            success();
        } else {
            success(template.getInstance().templateProperty());
        }
	},
	// Thread methods to start and stop
	startThread: function (success, fail) {
		success(template.getInstance().startThread());
	},
	stopThread: function (success, fail) {
		success(template.getInstance().stopThread());
	}
};