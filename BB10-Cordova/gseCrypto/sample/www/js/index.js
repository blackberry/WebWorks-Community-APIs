...
var app = {
	...
	testPluginCalls: function() {
		if (community && community.gsecrypto) {
			app.writeOut(community.gsecrypto.test());
			app.writeOut(community.gsecrypto.testInput('My Test Data'));
			app.writeOut('Template Property was: ' + community.gsecrypto.templateProperty);
			community.gsecrypto.templateProperty = 99;
			app.writeOut('Now: ' + community.gsecrypto.templateProperty);
			app.writeOut('Sent Async Request');
			var jsonData = {"value1":10,"value2":14};
			community.gsecrypto.testAsync(jsonData, app.aSyncCallback);
			community.gsecrypto.startThread(app.threadCallback);
		} else {
	...
	threadCallback: function(data) {
		if (app.threadStarted) {
			...
			if (json.threadCount >= 10) {
				var end = community.gsecrypto.stopThread();
				app.writeOut(end);
