...
std::string ExtractZIPFileNDK::templateTestString() {
	return "Template Test Function";
}

// Take in input and return a value
std::string ExtractZIPFileNDK::templateTestString(const std::string& inputString) {
	return "Template Test Function, got: " + inputString;
}

// Get an integer property
std::string ExtractZIPFileNDK::getTemplateProperty() {
	stringstream ss;
	ss << templateProperty;
	return ss.str();
}
...
void ExtractZIPFileNDK::setTemplateProperty(const std::string& inputString) {
	templateProperty = (int) strtoul(inputString.c_str(), NULL, 10);
}

// Asynchronous callback with JSON data input and output
void ExtractZIPFileNDK::templateTestAsync(const std::string& callbackId, const std::string& inputString) {
...
void* TemplateThread(void* parent) {
	ExtractZIPFileNDK *pParent = static_cast<ExtractZIPFileNDK *>(parent);
...
std::string ExtractZIPFileNDK::templateStartThread(const std::string& callbackId) {
	if (!m_thread) {
		int rc;
...
// Sets the stop value
std::string ExtractZIPFileNDK::templateStopThread() {
	int rc;
...
void ExtractZIPFileNDK::templateThreadCallback() {
	Json::FastWriter writer;
	Json::Value root;
...
bool ExtractZIPFileNDK::isThreadHalt() {
	int rc;
	bool isThreadHalt;
	rc = pthread_mutex_lock(&mutex);
