...
module.exports = {
	...
	startLed: function (success, fail, args, env) {
		...
		result.ok(led.getInstance().startLed(result.callbackId, args), false); 
	}

	stopLed: function (success, fail, args, env) {
		var result = new PluginResult(args, env); 
		result.ok(led.getInstance().stopLed(result.callbackId, args), false); 
	}
...
JNEXT.FlashLed = function () {
	...
	self.startLed = function (callbackId, input) {
		return JNEXT.invoke(self.m_id, "flashLedStartNative " + input);
	}

	self.stopLed = function (callbackId, input) {
